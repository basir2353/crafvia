import { diffText, type DiffLine } from './textProcess.js'
...
const CRON_FIELDS = ['minute', 'hour', 'dayOfMonth', 'month', 'dayOfWeek'] as const
...
export function processDiff(textA: string, textB: string): DevToolResponse {
  ...
  const added = diff.filter((d: DiffLine) => d.type === 'add').length
  const removed = diff.filter((d: DiffLine) => d.type === 'remove').length
  return {
    output: diff.map((d: DiffLine) => `${d.type === 'add' ? '+' : d.type === 'remove' ? '-' : ' '} ${d.line}`).join('\n'),
    meta: { diff, added, removed, unchanged: diff.filter((d: DiffLine) => d.type === 'equal').length },
  }
}
