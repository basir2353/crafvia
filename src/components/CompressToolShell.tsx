...
export function CompressToolShell({ config, children }: Props) {
      ...
      <main className="tool-main">
        <div className="tool-container">
          <nav className="breadcrumbs" aria-label="Breadcrumb">
            ...
            <Link to="/categories/compressors">{config.category}</Link>
            <ChevronRight size={14} aria-hidden />
...
export function CompressOutputActions({
  output,
  onClear,
  disabled,
...
export function formatSavings(originalBytes: number, outputBytes: number): string {
  ...
  const percent = Math.round((saved / originalBytes) * 100)
  const format = (bytes: number) => {
    if (bytes < 1024) return `${bytes} B`
    if (bytes < 1024 * 1024) return `${(bytes / 1024).toFixed(1)} KB`
    return `${(bytes / (1024 * 1024)).toFixed(2)} MB`
  }
  return `${format(originalBytes)} → ${format(outputBytes)} (${percent}% smaller)`
}
