...
import type { CalcToolConfig } from '../config/calcTools'
...
type Props = {
  config: CalcToolConfig
  children: ReactNode
}

export function CalcToolShell({ config, children }: Props) {
      ...
      <main className="tool-main">
        <div className="tool-container">
          <nav className="breadcrumbs" aria-label="Breadcrumb">
            ...
            <Link to="/categories/calculators">{config.category}</Link>
            <ChevronRight size={14} aria-hidden />
...
export function CalcOutputActions({
  output,
  onClear,
  disabled,
}: {
