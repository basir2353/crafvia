...
import type { SecurityToolConfig } from '../config/securityTools'
...
export function SecurityToolShell({ config, children }: Props) {
      ...
      <main className="tool-main">
        <div className="tool-container">
          <nav className="breadcrumbs" aria-label="Breadcrumb">
            ...
            <Link to="/categories/security-tools">{config.category}</Link>
            <ChevronRight size={14} aria-hidden />
...
export function SecurityOutputActions({
  output,
  onClear,
  disabled,
...
export function PasswordStrengthMeter({ score, label }: { score: number; label: string }) {
  const color =
    score >= 80 ? '#16a34a' : score >= 60 ? '#65a30d' : score >= 40 ? '#ca8a04' : score >= 20 ? '#ea580c' : '#dc2626'

  return (
    <div className="security-strength-meter">
      <div className="security-strength-bar" aria-hidden>
        <div className="security-strength-fill" style={{ width: `${score}%`, background: color }} />
      </div>
      <p className="tool-result-stats">
        <strong style={{ color }}>{label}</strong> · {score}/100
      </p>
    </div>
  )
}
